...
/**
 * DSLPFW_Local_Pickup_WooCommerce_Timezones helper class.
 *
 * @since 1.0.0
 */
class DSLPFW_Local_Pickup_WooCommerce_Timezones {
	 ...
	 * @since 1.0.0
	 *
	 * @param \DSLPFW_Local_Pickup_Location_Address $address an address object
	...
	public static function get_timezone_from_address( $address, $default = null ) {
		...
		try {
			...
			dslpfw()->dslpfw_log( sprintf( 'Error while parsing parse timezone identifier %1$s, will make another attempt using the site timezone (%2$s): %3$s', $timezone, $site_tz, $e->getMessage() ), 'info' );
			...
			try {
				...
				dslpfw()->dslpfw_log( sprintf( 'Error while parsing timezone identifier %1$s, will make another attempt in UTC: %2$s', $timezone, $e->getMessage() ), 'info' );
	...
	public static function get_timezone_identifier_from_address( $address, $default = null ) {
		...
		$country    = $address instanceof \DSLPFW_Local_Pickup_Location_Address ? $address->get_country() : '';
		$state      = $address instanceof \DSLPFW_Local_Pickup_Location_Address ? $address->get_state() : '';
		...
		switch ( $country ) {
			...
			case 'PT':
				...
				if ( $address instanceof \DSLPFW_Local_Pickup_Location_Address ) {
					...
					if ( str_contains( strtolower( $address_lines ), 'madeira' ) ) {
						$identifier = 'Atlantic/Madeira';
					} elseif ( str_contains( strtolower( $address_lines ), 'azores' ) ) {
		 ...
		 * @param \DSLPFW_Local_Pickup_Location_Address $address the related address to obtain the timezone from
		 */
		return (string) apply_filters( 'dslpfw_timezone_identifier', $identifier, $address );
	}
}