 ...
 * @since 1.0.0
 */
class DSLPFW_Local_Pickup_WooCommerce_Appointment extends \WC_Data {
	 ...
	 * @throws \Exception when the shipping item is not valid, is not a local pickup WooCommerce item or has no appointment data
	 */
	public function __construct( $id = 0 ) {
		...
		if ( $id && is_numeric( $id ) ) {
			...
			try {
				...
				throw new \Exception( $e->getMessage() ); //phpcs:ignore
			}
			...
			throw new \Exception( 'Invalid shipping item id.' );
		}
	}
	 ...
	 * @throws \Exception when the shipping item is not a local pickup WooCommerce item or has no appointment data
	 */
	private function get_data_from_shipping_item( \WC_Order_Item_Shipping $shipping_item ) {
		...
		if ( dslpfw_shipping_method_id() !== $shipping_item->get_method_id() ) {
			throw new \Exception( 'Invalid shipping item.' );
		}

		$order_items = dslpfw()->get_dslpfw_orders_object()->get_dslpfw_order_items_object();

		$start = $shipping_item->get_meta( '_dslpfw_pickup_appointment_start', true );
		$end   = $shipping_item->get_meta( '_dslpfw_pickup_appointment_end', true );
		...
		try {
			...
			if ( ! $start ) {
				...
				$date = wc_get_order_item_meta( $shipping_item->get_id(), '_dslpfw_pickup_date' );

				// legacy meta data used to store a value to offset the location's business hours for the given pickup day
				$time_offset = max( 0, (int) wc_get_order_item_meta( $shipping_item->get_id(), '_dslpfw_pickup_minimum_hours' ) );

				$start = $end = null;

				if ( empty( $date ) ) {
					throw new \Exception( 'The shipping item has no appointment data.' );
				}
            ...
            dslpfw()->dslpfw_log( 'Order: '.$shipping_item->get_order_id(). '| Exception: '.$e->getMessage(), 'info' );
		}
	 ...
	 * @throws \Exception if the start date set is invalid
	 */
	...
	public function get_end( $context = 'view' ) {
		...
		if ( $this->is_duration_type( self::DURATION_TYPE_ANYTIME_WITH_LEAD_TIME ) ) {
            ...
            $pickup_hours = $pickup_location->get_pickup_hours();

			if ( $pickup_location && $pickup_hours ) {
				$schedule = $pickup_hours->get_schedule();
			} else {
				$schedule = dslpfw_shipping_method()->get_default_pickup_hours();
			}
	 ...
	 * @since 1.0.0
	 *
	 * @param array|\DSLPFW_Local_Pickup_Location_Address $address pickup location address object or address data
	 */
	public function set_pickup_location_address( $address ) {
		...
		if ( ! $address instanceof \DSLPFW_Local_Pickup_Location_Address ) {
			$address = new \DSLPFW_Local_Pickup_Location_Address( is_array( $address ) ? $address : [] );
		}
	 ...
	 *
	 * @param string $context the request context - `edit` or `view`
	 * @return \DSLPFW_Local_Pickup_Location_Address
	 */
	public function get_pickup_location_address( $context = 'view' ) {
		...
		if ( ! $address instanceof \DSLPFW_Local_Pickup_Location_Address ) {
			$address = new \DSLPFW_Local_Pickup_Location_Address( [] );
		}
	...
	public function save() {
		...
		$this->set_order_item_meta_data( $order_item, '_dslpfw_pickup_appointment_start', $start ? $start->getTimestamp() : null );
		$this->set_order_item_meta_data( $order_item, '_dslpfw_pickup_appointment_end', $end ? $end->getTimestamp() : null );
		$this->set_order_item_meta_data( $order_item, '_dslpfw_pickup_location_id', $this->get_pickup_location_id() );
		$this->set_order_item_meta_data( $order_item, '_dslpfw_pickup_location_address', $this->get_pickup_location_address()->get_array() );
		$this->set_order_item_meta_data( $order_item, '_dslpfw_pickup_location_name', $this->get_pickup_location_name() );
		$this->set_order_item_meta_data( $order_item, '_dslpfw_pickup_location_phone', $this->get_pickup_location_phone() );
	 ...
	 */
	public function delete( $force_delete = false ) { //phpcs:ignore
		...
		try {
			...
			$success = wc_delete_order_item_meta( $shipping_item_id, '_dslpfw_pickup_appointment_start' ) && $success;
			$success = wc_delete_order_item_meta( $shipping_item_id, '_dslpfw_pickup_appointment_end' )   && $success;
			$success = wc_delete_order_item_meta( $shipping_item_id, '_dslpfw_pickup_location_id' )       && $success;
			$success = wc_delete_order_item_meta( $shipping_item_id, '_dslpfw_pickup_location_address' )  && $success;
			$success = wc_delete_order_item_meta( $shipping_item_id, '_dslpfw_pickup_location_name' )     && $success;
			$success = wc_delete_order_item_meta( $shipping_item_id, '_dslpfw_pickup_location_phone' )    && $success;
