...
/**
 * This class will use to handles shipping packages on cart and checkout.
 *
 * @since 1.0.0
 */
class DSLPFW_Local_Pickup_WooCommerce_Packages {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_cart_shipping_packages', array( $this, 'dslpfw_handle_packages' ), 1 );

		// // filter again the shipping packages to toggle Local Pickup WooCommerce available from available rates
		add_filter( 'woocommerce_shipping_packages', array( $this, 'dslpfw_filter_package_rates' ), 1 );
    }
    ...
    /**
	 * Filter package rates to remove Local Pickup WooCommerce option for packages that cannot be picked up and remove shipping options for packages that must be picked up.
	 *
	 * @since 1.0.0
	 *
	 ...
	 * @return array
	 */
	public function dslpfw_filter_package_rates( $packages ) {

        $ds_local_pickup        = dslpfw_shipping_method();
		$pickup_shipping_id     = $ds_local_pickup->get_method_id();
		$append_ship_only_items = array();
		$pickup_packages        = array();

        if ( ! empty( $packages ) && $ds_local_pickup->is_available() ) {

			foreach ( $packages as $index => $package ) {
                if ( ! isset( $package['ship_via'] ) && isset( $package['rates'][ $pickup_shipping_id ] ) ) {
                    
                    if ( ! $this->package_can_be_shipped( $package ) || ( isset( $package['contents'] ) && is_array( $package['contents'] ) && $this->package_should_be_picked_up( $package ) ) ) {

                        // so we don't unset Local Pickup Rates, however, we need to check if there are any items that cannot be picked up
						foreach ( $package['contents'] as $item_key => $item ) {

							if ( isset( $item['data'] ) && ! dslpfw_product_can_be_picked_up( $item['data'] ) ) {
						...
						$packages[ $index ]['ship_via'] = $package['ship_via'] = array( $pickup_shipping_id );
						// remove shipping options
						$packages[ $index ]['rates'] = $ds_local_pickup->get_rates_for_package( $package );

                    } elseif ( ! $this->package_can_be_picked_up( $package ) && $this->package_can_be_shipped( $package ) ) {

						// remove pickup option
						unset( $packages[ $index ]['rates'][ $pickup_shipping_id ] );
					...
					}  elseif ( ! $this->package_should_be_picked_up( $package ) && ( $ds_local_pickup->dslpfw_is_per_item_selection_enabled() || ( $ds_local_pickup->dslpfw_is_per_order_selection_enabled() && $ds_local_pickup->is_item_handling_mode( 'customer' ) ) ) ) {
                    ...
                    } elseif ( isset( $package['ship_via'] ) && in_array( $pickup_shipping_id, $package['ship_via'], true ) ) {
			...
			if ( count( $pickup_packages ) > 1  && $ds_local_pickup->dslpfw_is_per_order_selection_enabled() && $ds_local_pickup->is_item_handling_mode( 'automatic' ) ) {
                ...
                // refresh rates
                $rates = $ds_local_pickup->get_rates_for_package( $grouped_pickup_package );
	...
	public function package_can_be_picked_up( $package ) {
		...
		if ( isset( $package['contents'] ) && is_array( $package['contents'] ) ) {
			...
			foreach ( $package['contents'] as $item ) {
				...
				if ( isset( $item['data'] ) && $item['data'] instanceof \WC_Product && ! dslpfw_product_can_be_picked_up( $item['data'] ) ) {
	...
	public function package_can_be_shipped( $package ) {
		...
		if ( ! $can_be_shipped ) {
			...
			if ( $ds_local_pickup->dslpfw_is_per_order_selection_enabled() && $ds_local_pickup->is_default_handling( 'ship' ) ) {
	 ...
	 *
	 * @see DSLPFW_Local_Pickup_WooCommerce_Packages::dslpfw_filter_package_rates()
	 *
	 * @since 1.0.0
	 *
	...
	private function package_should_be_picked_up( $package ) {
		...
		$only_dslpfw_available = isset( $package['rates'] ) && 1 === count( $package['rates'] ) && $ds_local_pickup->get_method_id() === key( $package['rates'] );
		...
		if ( $ds_local_pickup && $only_dslpfw_available && $ds_local_pickup->dslpfw_is_per_order_selection_enabled() && $ds_local_pickup->is_default_handling( 'pickup' ) ) {
	...
	public function get_rates_for_package( $package ) {
		...
		if ( ! empty( $package['contents'] ) && $shipping_zone ) {
	 ...
	 * @return array
	 */
	public function dslpfw_handle_packages( $packages ) {

        $ds_local_pickup    = dslpfw_shipping_method();
		$pickup_data        = dslpfw()->get_dslpfw_session_object()->get_cart_item_pickup_data( null );
        
        if ( ! empty( $packages ) && $ds_local_pickup->is_available() ) {

            $shipping_rates        = $this->get_rates_for_package( $packages[0] );
            $package_pickup_data   = dslpfw()->get_dslpfw_session_object()->get_package_pickup_data( 0 );
            $new_packages          = array();
			 ...
			 */
			$items = (array) apply_filters( 'dslpfw_cart_shipping_packages', array(
			...
			if ( ! empty( $items['pickup_items'] ) ) {
				...
				foreach ( $same_pickup_locations as $pickup_location_id => $pickup_items ) {
					...
					} elseif ( $package_data = dslpfw()->get_dslpfw_session_object()->get_package_pickup_data( $index ) ) { //phpcs:ignore
					...
					dslpfw()->get_dslpfw_session_object()->set_package_pickup_data( $index, [
					...
					if ( $ds_local_pickup->dslpfw_is_per_item_selection_enabled() || ( $ds_local_pickup->dslpfw_is_per_order_selection_enabled() && $ds_local_pickup->is_item_handling_mode( 'customer' ) ) ) {
						$new_packages[ $index ]['ship_via'] = [ $ds_local_pickup->get_method_id() ];
					}
			...
			if ( ! empty( $items['ship_items'] ) ) {
				...
				dslpfw()->get_dslpfw_session_object()->delete_package_pickup_data( $index );
			}
	...
	private function get_cart_item_pickup_location_id( $cart_item ) {
		...
		$available_locations = $product ? dslpfw()->get_dslpfw_products_object()->get_product_pickup_locations( $product, array( 'fields' => 'ids' ) ) : null;
	...
	private function get_cart_item_common_pickup_location_id( array $contents, $pickup_data = array() ) {
		...
		foreach ( $contents as $item ) {
			...
			if ( $package_product instanceof \WC_Product ) {
				...
				$available_locations = dslpfw()->get_dslpfw_products_object()->get_product_pickup_locations( $package_product, array( 'fields' => 'ids' ) );
				$location_ids[]      = 1 === count( $available_locations ) ? (int) current( $available_locations ) : 0;
			}
		}

		$location_ids    = array_unique( $location_ids );
		$pickup_location = 1 === count( $location_ids ) ? dslpfw_get_pickup_location( current( $location_ids ) ) : null;

		// determine if the cart item should "inherit" a location from other cart items in per-order location mode
		if ( ! $pickup_location && ! empty( $pickup_data ) && dslpfw_shipping_method()->dslpfw_is_per_order_selection_enabled() ) {

			foreach ( $pickup_data as $item_pickup_data ) {

				if ( ! empty( $item_pickup_data['pickup_location_id'] ) ) {
					$pickup_location = dslpfw_get_pickup_location( $item_pickup_data['pickup_location_id'] );
					break;
				}
			}
		}
	 ...
	 *
	 * @see DSLPFW_Local_Pickup_WooCommerce_Packages::dslpfw_handle_packages()
	 *
	 * @since 1.0.0
	 *
	...
	private function cart_item_should_be_picked_up( $cart_item, $cart_item_key, $pickup_data = array(), $shipping_rates = array() ) {
		...
		if ( array_key_exists( $cart_item_key, $pickup_data ) && isset( $pickup_data[ $cart_item_key ]['handling'] ) && 'pickup' === $pickup_data[ $cart_item_key ]['handling'] ) {
			...
			if ( $has_item_data && ! dslpfw_product_can_be_picked_up( $cart_item['data'] ) ) {
				$pickup = false;
			}
			...
			if ( $has_item_data && dslpfw_shipping_method()->dslpfw_is_per_order_selection_enabled() && dslpfw_shipping_method()->is_item_handling_mode( 'automatic' ) && $this->is_shipping_required() && ! $this->are_shipping_and_pickup_required() ) {
				$pickup = false;
			}
		...
		// sanity check if the cart item must be picked up by product setting
			...
			$can_be_picked_up  = dslpfw_product_can_be_picked_up( $cart_item['data'] );
			$must_be_picked_up = dslpfw_product_must_be_picked_up( $cart_item['data'] );
			$ds_local_pickup   = dslpfw_shipping_method();
			...
			$pickup = $can_be_picked_up
			          && ( $must_be_picked_up
			               ...
			               || ( $ds_local_pickup->dslpfw_is_per_order_selection_enabled()
			                    && ! $ds_local_pickup->is_item_handling_mode( 'automatic' )
			                    && $ds_local_pickup->is_default_handling( 'pickup' )
			               ...
			               || ( $ds_local_pickup->dslpfw_is_per_order_selection_enabled()
			                    && $ds_local_pickup->is_item_handling_mode( 'automatic' )
			                    && $ds_local_pickup->is_default_handling( 'ship' )
    ...
    /**
	 * Checks if the product combination in the cart requires at least one shipping package.
	 *
	 * @since 1.0.0
	 *
	 * @return bool
	...
	public function is_shipping_required() {
		...
		foreach ( $cart_contents as $cart_item ) {
			...
			if ( isset( $cart_item['data'] ) && $cart_item['data'] instanceof \WC_Product ) {
				...
				if ( dslpfw_product_must_be_shipped( $cart_item['data'] ) ) {
    ...
    /**
	 * Checks if the product combination in the cart requires at least one pickup package.
	 *
	 * @since 1.0.0
	 *
	 * @return bool
	...
	public function is_pickup_required() {
		...
		foreach ( $cart_contents as $cart_item ) {
			...
			if ( isset( $cart_item['data'] ) && $cart_item['data'] instanceof \WC_Product ) {
				...
				if ( dslpfw_product_must_be_picked_up( $cart_item['data'] ) ) {
	...
	public function get_package_pickup_location( $package = array() ) {
		...
		return dslpfw_get_pickup_location( $pickup_location_id );
	}
	...
	public function get_package_only_pickup_location_id( $package = array() ) {
		...
		if ( is_array( $package ) && ! empty( $package ) && isset( $package['contents'] ) && is_array( $package['contents'] ) ) {
	 ...
	 * However, we have a WooCommerce session and a Local Pickup session running alongside with it for necessary internal purposes.
	 * By default, this method will give priority at the WooCommerce session, and if not set or empty, will look at the Local Pickup session for counting.
	 * Given the many ways Local Pickup can work at checkout and exception cases, this may lead to ambiguity or disparity between the possible counts.
	 ...
	 * @since 1.0.0
	 *
	 * @param array $packages optional packages (defaults to packages in session: WC, if set first, or fallback to DSLPFW session)
	 * @return int
	 */
	public function get_packages_for_pickup_count( $packages = array() ) {
        ...
        $packages = $this->get_packages_for_counting( $packages );
		...
		if ( $packages ) {
			...
			foreach( $packages as $package ) {
				...
				} elseif ( ! empty( $package['ship_via'] ) && is_array( $package['ship_via'] ) && 1 === count( $package['ship_via'] ) && in_array( dslpfw_shipping_method_id(), $package['ship_via'], false ) ) { //phpcs:ignore
					$pickup_packages++;
				}
			}
		}
	...
	private function get_packages_for_counting( $packages = array() ) {
		...
		if ( empty( $packages ) || ! is_array( $packages ) ) {
			...
			if ( empty( $packages ) ) {
				$packages = wc()->session->get( 'wc_ds_local_pickup_packages' );
			}

			if ( empty( $packages ) ) {
				$packages = wc()->session->get( 'wc_ds_local_pickup_cart_items' );
			}
		}
